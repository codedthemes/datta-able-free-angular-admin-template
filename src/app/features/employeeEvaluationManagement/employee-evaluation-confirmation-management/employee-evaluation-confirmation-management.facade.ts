...
export class EmployeeEvaluationConfirmationFacade {
    ...
    constructor(
        ...
        private employeeEvaluationConfirmationServices: EmployeeEvaluationConfirmationServices
    ) {
    }
    deleteBank(id: string): void {
        // const deleteBankProcess$ = this.employeeEvaluationConfirmationServices.DeleteBank(id).pipe(
        //     tap(res => {
    ...
    GetBanks(): any {
        const getBanksProcess$ = this.employeeEvaluationConfirmationServices.GetBanks(1).pipe(
            tap(res => {
                // if (res.type == ResponseType.Success) {
    ...
    AddBank(Bank: any): void {
        const addBankProcess$ = this.employeeEvaluationConfirmationServices.AddBank(Bank).pipe(
            tap(res => {
                // if (res.type == ResponseType.Success) {
    ...
    UpdateBank(Bank: any): void {
        const updateBankProcess$ = this.employeeEvaluationConfirmationServices.UpdateBank(Bank).pipe(
            // tap(res => {
            //     if (res.type == ResponseType.Success) {
